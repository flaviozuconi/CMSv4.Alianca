...
public class RoxyFilemanHandler : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    protected Dictionary<string, string> ParseJSON(string file){
        ...
        string json = "";
        string defaultDir = "/portal/" + BLPortal.Atual.Diretorio + "/arquivos/imagens";

        if(!Directory.Exists(HttpContext.Current.Request.MapPath(defaultDir)))
            Directory.CreateDirectory(HttpContext.Current.Request.MapPath(defaultDir));

        try{
            json = File.ReadAllText(_context.Server.MapPath(file), System.Text.Encoding.UTF8);

            json = json.Replace("%%DIR%%", defaultDir);
            //Substitui a tag pelo dir para o portal
        }
        catch(Exception ex){}
    ...
    private void _copyDir(string path, string dest){
        if(!Directory.Exists(dest))
        {
            ...
            BLReplicar.Diretorio(BLReplicar.ReverseMapPath(dest));
        }
        ...
        foreach(string f in  Directory.GetFiles(path))
        {
            ...
            if (!File.Exists(Path.Combine(dest, file.Name))){
                ...
                BLReplicar.Arquivo(Path.Combine(dest, file.Name));
            }
        }
    ...
    protected void CopyFile(string path, string newPath)
    {
        ...
        if (!file.Exists)
            ...
            try{
                ...
                BLReplicar.Arquivo(Path.Combine(newPath, newName));
                _r.Write(GetSuccessRes());
            }
            catch(Exception ex){
    ...
    protected void CreateDir(string path, string name)
    {
        ...
        if(!Directory.Exists(path))
            ...
            try
            {
                ...
                BLReplicar.Diretorio(BLReplicar.ReverseMapPath(path));
                _r.Write(GetSuccessRes());
            }
            catch (Exception ex)
            {
    ...
    protected void DeleteDir(string path)
    {
        ...
        else
        {
            try
            {
                ...
                BLReplicar.ExcluirDiretoriosReplicados(path);
                _r.Write(GetSuccessRes());
            }
            catch (Exception ex)
            {
    ...
    protected void DeleteFile(string path)
    {
        ...
        else
        {
            ...
            {
                File.Delete(path);
                BLReplicar.ExcluirArquivosReplicados(path);
                _r.Write(GetSuccessRes());
            }
            catch (Exception ex)
            {
    ...
    protected void MoveDir(string path, string newPath)
    {
        ...
        if(dest.FullName.IndexOf(source.FullName) == 0)
            ...
            try{
                BLReplicar.MoverDiretorio(source.FullName, dest.FullName);
    ...
    protected void MoveFile(string path, string newPath)
    {
        ...
        else
        {
            try
            {
                BLReplicar.MoverArquivo(source.FullName, dest.FullName);
                source.MoveTo(dest.FullName);
                _r.Write(GetSuccessRes());
            }
            catch (Exception ex)
    ...
    protected void RenameDir(string path, string name)
    {
        ...
        else
        {
            try
            {
                BLReplicar.RenomearDiretorio(BLReplicar.ReverseMapPath(source.FullName), BLReplicar.ReverseMapPath(dest.FullName));
                source.MoveTo(dest.FullName);
                _r.Write(GetSuccessRes());
            }
            catch (Exception ex)
    ...
    protected void RenameFile(string path, string name)
    {
        ...
        else
        {
            try
            {
                BLReplicar.RenomearArquivo(source.FullName, dest.Name);
                source.MoveTo(dest.FullName);
                _r.Write(GetSuccessRes());
            }
            catch (Exception ex)
